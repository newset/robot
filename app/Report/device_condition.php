...
*/
  ...
  if($hospitalid!=""&&! filter_var($hospitalid, FILTER_VALIDATE_INT))
  {
     die("invalid hospitalid");
  }
  $query="DROP VIEW IF EXISTS `device_condition`;";
  $mysqli->query($query);
  //该时间应该是一个时间段，租赁时间在这之内都应该包含
  //根据1、只选择医院 2、只选择代理商 3、二者都选取交集 创建三种视图
  if($agencyid=="")
  $query = "create view device_condition as select i_robot.status,robot_id,i_hospital.name as hospital_name,i_agency.name as agency_name,lease_type_id,lease_started_at,lease_ended_at
            from i_robot_lease_log,i_hospital,i_agency,i_robot
			...
			and hospital_id=i_hospital.id 
			and i_robot.id=robot_id
			and agency_id=i_agency.id
			and i_hospital.province_id=$provinceid and i_hospital.city_id=$cityid
			order by robot_id,agency_name,hospital_name;";
  else if($hospitalid=="")
  $query = "create view device_condition as select i_robot.status,robot_id,i_hospital.name as hospital_name,i_agency.name as agency_name,lease_type_id,lease_started_at,lease_ended_at
            from i_robot_lease_log,i_hospital,i_agency,i_robot
  ...
  $query = "create view device_condition as select i_robot.status,robot_id,i_hospital.name as hospital_name,i_agency.name as agency_name,lease_type_id,lease_started_at,lease_ended_at
            from i_robot_lease_log,i_hospital,i_agency,i_robot
	        where agency_id=$agencyid and hospital_id=$hospitalid
			(lease_started_at>='$starttime' and lease_started_at<='$endtime' or
			 lease_ended_at>='$starttime' and lease_ended_at<='$endtime' or
			 lease_started_at<='$starttime' and lease_ended_at>='$endtime' )
			and hospital_id=i_hospital.id
  ...
  /* 手术总数 */
  /*设备编号 地区 代理商 医院 历史总数 销售状态 工作状态 代理开始时间 代理结束时间 */
  $query=" select distinct maintable.robot_id,$provinceid as province_id,$cityid as city_id,maintable.agency_name,maintable.hospital_name,
          maintable.lease_started_at,maintable.lease_ended_at,maintable.lease_type_id,maintable.status,ifnull(subtable.num,0) as hissurgerynum 
		  from device_condition as maintable left join
  ...
  echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
		...
		<body>
		  ...
		  <table id="show" class="gridtable" >
			<caption style="font-family:Arial, Helvetica;text-align:left;font-size:30px;text-align:center">设备状态清单</caption>
			<thead>
				<tr ><th>设备编号</th><th>省份id </th><th>城市id </th><th>代理商 </th><th>医院 </th><th>手术总数 </th><th>历史总数 </th><th>维修次数 </th>
				<th>销售状态 </th><th>工作状态</th><th>代理开始时间 </th><th>代理结束时间 </th></tr>
			</thead>
			<tbody>  ';
  ...
  while($hissurgeryrow = $hissurgeryresult->fetch_array()
	...
	{//$s代表show
	  ...
	  $ssalestatus = $hissurgeryrow['lease_type_id'];
	  $sworkstatus = $hissurgeryrow['status'];//工作状态
      $sleasestart = $hissurgeryrow['lease_started_at'];
	  $sleaseend = $hissurgeryrow['lease_ended_at'];
	 ...
	 //按行显示至表格
	  echo "<tr><td>$srobotid</td><td> $sprovinceid </td><td> $scityid </td> <td>$sagencyname </td><td> $shospitalname</td>
		   ...
		   <td> $ssalestatus </td> <td>$sworkstatus </td> <td>$sleaseend </td>  <td>$sleasestart </td>
	  </tr>";
	  $num++;
	}
	    $tssurgerynum=0;
	...
	{//算出各num总数,变量名前加ts，t代表total  s代表show
		...
		<td style='border-left-width:0;border-right-width:0'> </td><td style='border-left-width:0'> </td><td> $tssurgerynum </td>
		   ...
		   <td style='border-left-width:0;border-right-width:0'> </td> <td style='border-left-width:0;border-right-width:0'> </td>
