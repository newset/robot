...
*/
  ...
  /* 创建该地区的代理商视图 */
  $query="DROP VIEW IF EXISTS `robot_sale`;";
  $mysqli->query($query);
  $query = "create view robot_sale as select id,name,name_in_charge,phone
            from i_agency
	        where city_id=$cityid and province_id=$provinceid 
			order by name;";
  $viewresult = $mysqli->query($query);
  
  /* 代理商 地区 联系人 电话 机器购买总数 */
  $query = "select maintable.id,maintable.name,maintable.name_in_charge,maintable.phone,ifnull(subtable.num,0) as robotbuynum 
           from robot_sale as maintable left join 
		   (select agency_id,count(id) as num from i_robot_lease_log where lease_started_at>='$starttime' and lease_ended_at<='$endtime' and lease_type_id=1 group by agency_id) as subtable 
            on maintable.id=subtable.agency_id
			order by name;";
  $robotbuyresult=$mysqli->query($query);
  /* Mark购买总数 */
  $query = "select maintable.name,ifnull(subtable.num,0) as markbuynum from robot_sale as maintable left join 
  ...
  /* 历史消耗Mark */
  $query = " select maintable.name,ifnull(subtable.num,0) as historyusenum from robot_sale as maintable left join 
  ...
  /* 消耗Mark */
  $query = " select maintable.name,ifnull(subtable.num,0) as usenum from robot_sale as maintable left join 
  ...
  echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
		...
		<body>
		  ...
		  <table id="show" class="gridtable" >
			<caption style="font-family:Arial, Helvetica;text-align:left;font-size:30px;text-align:center">机器人销售情况表</caption>
			<thead>
				<tr ><th>代理商</th><th>省份id </th><th>城市id </th><th>联系人 </th><th>电话</th><th>机器购买总数 </th><th>医院总数 </th><th>Mark购买总数 </th>
